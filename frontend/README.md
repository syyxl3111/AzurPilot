...
顶部导航与页面操作栏使用纯色背景（`var(--surface)`），确保清晰的文字对比度与轻量渲染；侧边栏、常驻面板及悬浮弹出菜单采用统一的毛玻璃材质（`var(--glass-tint)`、`backdrop-filter: blur(24px)`）。表单、日志和统计卡片使用稳定的实色表面；系统蓝表示主要操作，绿色表示运行状态。深色模式沿用系统设置中的主题选择。

背景由 `https://api.yppp.net/api.php` 提供，浏览器每次打开或刷新页面时请求一次，应用内导航复用图片。请求不携带页面来源；图片加载失败时保留渐变底色。减少透明度、高对比度与强制颜色模式隐藏背景图并使用实色材质。

`src/styles/apple.css` 统一导航、工具栏和各页面的视觉样式，`Wallpaper` 负责背景图片失败降级。
